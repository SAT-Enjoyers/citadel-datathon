...
def get_abbreviations():
    ...
    return pd.Series(df_abbrev['Abbreviation'].values, index=df_abbrev['State'].str.lower()).to_dict()
...
def plot_state_data(df: pd.DataFrame, val_column: str, title: str = None, col_title: str = None) -> plt.plot:
    ...
    gdf['value_color'] = gdf[val_column].apply(lambda x: mcolors.to_hex(mapper.to_rgba(x)))
    ...
    sm = plt.cm.ScalarMappable(cmap="YlOrBr", norm=plt.Normalize(vmin=v_min, vmax=v_max))
    sm._A = []
...
if __name__ == "__main__":
    ...
    plt = plot_state_data(df, val_column, "Title", "Color title")
    plt.savefig("geo/test.png")
